...
func (sp *ServiceProcessor) applyCustomIPAlloc(alloc *ipalloc.CustomIPAllocation) (
	...
	for _, ci := range alloc.CustomInterfaces {
		...
		if isLocal && ci.Network == defaultPodNetwork || ci.Network == "" {
			...
			ifName, _, ifExists := sp.IPNet.GetPodCustomIfName(podID.Namespace, podID.Name, ci.Name)
			if !ifExists {
	...
	if redirected {
		if isLocal && serviceEndpointIf != nil {
			...
			ifName, _, ifExists := sp.IPNet.GetPodCustomIfName(podID.Namespace, podID.Name, serviceEndpointIf.Name)
			if !ifExists {
